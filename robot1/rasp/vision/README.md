...
#### 1. `localization.py` — Core Processing Engine
...
#### 2. `gui.py` — Visualization & Debug Interface
...
_update_pose_from_scan()       # Pull latest pose from localization
```

**Important Change** (April 20, 2026):
- ✅ Removed ~180 lines of dead beacon detection code
- ✅ Now only displays pre-calculated beacons from localization
...
**Note**: Mostly superseded by localization + navigation_bridge
...
### LiDAR Connection (`localization.py`)
...
LiDAR Hardware
    ↓
┌─────────────────────────────────────┐
│   localization.py (Background Thread) │
├─────────────────────────────────────┤
│ • RPLidar acquisition                │
│ • Beacon clustering                  │
│ • SVD Umeyama trilateration          │
...
└──────────────────┬──────────────────┘
        ...
        └──┬────────────┬───────────┬──┘
           ↓            ↓           ↓
   ┌──────────────┐ ┌──────────────┐ ┌──────────────────┐
   │ gui.py │ │ robot.py     │ │ lidar_navigation │
...
2. Update PORT in `localization.py:L50`
...
**Fix**:
1. Verify robot position relative to beacons (world.py)
2. Check LiDAR scan in GUI (`python -m vision`)
...
- [ ] GUI launches and shows scan data (`python -m vision`)
...
**Module Authority**: `robot1/rasp/vision/`  
