...
**When Called:**
- Polling function from `localization.py::get_latest_scan_data()`
...
**When Called:**
- Polling function from `localization.py::get_latest_beacon_candidates()`
- Default: ~20 Hz polling in main loop
...
**When Called:**
- Polling function from `localization.py::get_corrected_pose()`
- Default: ~20 Hz polling in main loop
...
class Robot:
    ...
    def update(self):
        ...
        from vision.localization import update_teensy_pose
        update_teensy_pose(teensy_x, teensy_y, teensy_theta)
        
        # Step 3: Get LiDAR-corrected position
        from vision.localization import get_corrected_pose
...
ExecStart=/usr/bin/python3 /home/pi/CDR-Nantes/robot1/rasp && python -m telemetry.rerun_bridge --mode serve --host 0.0.0.0 --port 9876 --with-lidar
Restart=on-failure
RestartSec=5
