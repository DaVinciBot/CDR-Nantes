...
class MockTeensyCOM:
    ...
    def __init__(self):
        ...
        self.running = False
        self.thread = None
        logger.info("[OK] Mock Teensy COM initialisé")
    ...
    def send_bytes(self, data):
        ...
        if msg_id == MockMessages.SET_ODOMETRIE.value and len(data) >= 25:
            ...
            logger.info(f"[SIM] SET_ODOMETRIE reçu: X={x:.1f}, Y={y:.1f}, theta={theta:.3f}")
        ...
        if msg_id == MockMessages.SET_TARGET_POSITION.value and len(data) >= 25:
            ...
            with self._lock:
                self.target_x, self.target_y, self.target_theta = x, y, theta
            logger.debug(f"[SIM] SET_TARGET_POSITION: X={x:.1f}, Y={y:.1f}, theta={theta:.3f}")
            return
...
class MockLidar:
    ...
    def __init__(self, team_color="YELLOW"):
        ...
        logger.info(f"[OK] Mock Lidar initialisé ({team_color})")
...
class MockStrategy:
    ...
    def __init__(self, team_color="YELLOW"):
        ...
        logger.info(f"[OK] Mock Stratégie initialisée ({team_color})")
...
def inject_mocks():
    ...
    logger.info("[OK] Mocks injectés dans sys.modules")
...
def patch_robot_for_sim(robot_class):
    ...
    def patched_init(self, couleur_equipe):
        ...
        # Aucun GPIO en simulation. Sans ces deux attributs, stopper_tout()
        # levait AttributeError a la toute fin du match simule (11/09/2026).
        self._pin_tirette = None
        self._pin_couleur = None
...
def main_sim(with_rerun=False, rerun_mode="local", rerun_port=9876):
    ...
    if with_rerun:
        try:
            ...
            if rerun_mode == "serve":
                ...
                logger.info(f"[OK] Rerun serveur lancé sur port {rerun_port}")
                ...
                logger.info("[OK] Rerun viewer spawné localement")
            ...
            logger.info("[OK] Rerun Bridge opérationnel")

        except Exception as e:
            logger.warning(f"[WARN]  Rerun désactivé : {e}")
    ...
    logger.info("  Simulation sans tirette — démarrage immédiat...")
    ...
    try:
        ...
        logger.warning("\n[WARN]  Arrêt d'urgence (Ctrl+C)")

    finally:
        logger.info("\nDésactivation des systèmes...")
        mon_robot.stopper_tout()
        logger.info("[OK] Simulation terminée.")
