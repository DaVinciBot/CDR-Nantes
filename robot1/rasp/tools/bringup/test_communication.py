...
def handle_position(data: bytes) -> None:
    ...
    if len(data) >= 24:
        ...
        messages_received += 1
        logger.info(f" Position reçue #{messages_received}: X={x:.2f}mm, Y={y:.2f}mm, theta={theta:.4f}rad")
    else:
        logger.warning(f"[WARN]  Message trop court: {len(data)} bytes")

def main():
    global messages_sent   # without this, the first "+= 1" raises UnboundLocalError
    ...
    for x, y, theta, description in positions:
        ...
        com.send_bytes(msg)
        messages_sent += 1
        logger.info(f" Commande envoyée: X={x}mm, Y={y}mm, theta={theta}rad")
        time.sleep(3)
    ...
    if messages_received > 0:
        logger.info("\n[OK] SUCCÈS : La communication Rasp <-> Teensy fonctionne !")
        ...
        logger.warning("\n[WARN]  ATTENTION : Aucun message reçu de la Teensy")
